...
async fn source_watch_scan_imports_changed_files_and_enqueues_ingest_tasks() {
    ...
    assert_eq!(
        config_payload.get("enabled").and_then(Value::as_bool),
        Some(true)
    );
    assert_eq!(
    ...
    for _ in 0..10 {
        if !tasks::scheduler::run_scheduler_tick(&state)
            .await
            .unwrap_or(false)
        {
            break;
    ...
    assert!(
        project_root
            .join("raw/sources/team-a/docs/peer.md")
            .is_file()
    );
...
async fn source_watch_scan_copies_files_without_ingest_when_auto_ingest_is_disabled() {
    let env = TestEnvironment::start("source-watch-no-auto-ingest")
        .await
        .unwrap();
    let config = AppConfig::for_tests(env.database_url.clone(), env.redis_url.clone());
    let state = bootstrap_state_without_scheduler(&config).await.unwrap();
    ...
    let project_root = tempdir()
        .unwrap()
        .path()
        .join("source-watch-no-auto-ingest-project");
    let project_id = create_project(state.clone(), &cookie, &csrf, project_root.clone()).await;

    let watch_root = tempdir()
        .unwrap()
        .path()
        .join("watched-sources-no-auto-ingest");
    ...
    assert!(
        !tasks
            .iter()
            .any(|task| task.task_type == "project.ingest_source")
    );
}
