...
async fn ingest_creates_review_items_and_review_endpoint_can_update_status() {
    ...
    let reviews = reviews_payload
        .get("reviews")
        .and_then(Value::as_array)
        .unwrap();
    assert_eq!(reviews.len(), 1);
    let review_id = reviews[0]
        .get("id")
        .and_then(Value::as_str)
        .unwrap()
        .to_string();
    assert_eq!(
        reviews[0].get("status").and_then(Value::as_str),
        Some("open")
    );
...
async fn provider_generated_review_blocks_are_persisted_during_ingest() {
    ...
    let _env = TestEnvironment::start("provider-review-items")
        .await
        .unwrap();
    let mock = MockOpenAiServer::start(MockScenario::ingest_with_review_success())
        .await
        .unwrap();
    let config = AppConfig::for_tests(_env.database_url.clone(), _env.redis_url.clone());
    let state = bootstrap_state(&config).await.unwrap();
...
async fn ingest_runs_dedicated_review_stage_when_generation_has_no_review_blocks() {
    let temp = tempdir().unwrap();
    let _env = TestEnvironment::start("provider-dedicated-review-stage")
        .await
        .unwrap();
    let mock = MockOpenAiServer::start(MockScenario::ingest_with_dedicated_review_stage_success())
        .await
        .unwrap();
    let config = AppConfig::for_tests(_env.database_url.clone(), _env.redis_url.clone());
    let state = bootstrap_state(&config).await.unwrap();
...
async fn final_ingest_triggers_review_sweep_for_existing_missing_page_items() {
    ...
    assert_eq!(
        task.result
            .as_ref()
            .and_then(|result| result.get("reviewSweep")),
        Some(&json!({
...
async fn final_ingest_uses_provider_to_semantically_resolve_remaining_reviews() {
    ...
    let mock = MockOpenAiServer::start(MockScenario::ingest_then_sweep_llm_success())
        .await
        .unwrap();
    let config = AppConfig::for_tests(_env.database_url.clone(), _env.redis_url.clone());
    let state = bootstrap_state(&config).await.unwrap();
...
async fn review_endpoint_supports_status_type_and_limit_filters() {
    ...
    let default_reviews = default_payload
        .get("reviews")
        .and_then(Value::as_array)
        .unwrap();
    ...
    assert!(
        default_reviews
            .iter()
            .all(|item| item.get("status").and_then(Value::as_str) != Some("resolved"))
    );
    ...
    let filtered_reviews = filtered_payload
        .get("reviews")
        .and_then(Value::as_array)
        .unwrap();
    assert_eq!(filtered_reviews.len(), 1);
    assert_eq!(
        filtered_reviews[0].get("id").and_then(Value::as_str),
        Some("review-1")
    );
}
