...
async fn graph_returns_nodes_and_edges_from_wikilinks() {
  ...
  .unwrap();
    ...
    let payload = read_json(response.into_body()).await;
    assert_eq!(
        payload.get("nodes").and_then(Value::as_array).map(Vec::len),
        Some(2)
    );
    assert_eq!(
        payload.get("edges").and_then(Value::as_array).map(Vec::len),
        Some(1)
    );
}
...
async fn graph_supports_link_counts_weights_query_filter_and_limit() {
  ...
  .unwrap();
    ...
    let nodes = graph_payload
        .get("nodes")
        .and_then(Value::as_array)
        .unwrap();
    let edges = graph_payload
        .get("edges")
        .and_then(Value::as_array)
        .unwrap();
    ...
    let filtered_nodes = filtered_payload
        .get("nodes")
        .and_then(Value::as_array)
        .unwrap();
...
async fn graph_supports_node_type_filter() {
    let temp = tempdir().unwrap();
    let _env = TestEnvironment::start("graph-node-type-query")
        .await
        .unwrap();
    let config = AppConfig::for_tests(_env.database_url.clone(), _env.redis_url.clone());
    let state = bootstrap_state(&config).await.unwrap();
