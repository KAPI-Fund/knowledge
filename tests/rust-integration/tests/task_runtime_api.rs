...
async fn startup_requeues_running_and_retry_waiting_tasks() {
    ...
    store::update_task_status(&state, &running.id, "running")
        .await
        .unwrap();
    store::update_task_status(&state, &waiting.id, "retry_waiting")
        .await
        .unwrap();

    knowledge_server::tasks::recovery::recover_tasks(&state)
        .await
        .unwrap();
...
async fn queued_task_creation_emits_worker_wakeup_signal() {
    ...
    assert_eq!(response.status(), StatusCode::ACCEPTED);
    let wakeup: Option<String> = state.cache.get_json("tasks:wakeup").await.unwrap();
...
async fn retry_waiting_tasks_are_not_reacquired_before_next_retry_at_and_reenter_once_due() {
    ...
    let early = store::acquire_next_task(&state, "worker-1", 30)
        .await
        .unwrap();
    ...
    let due = store::acquire_next_task(&state, "worker-1", 30)
        .await
        .unwrap()
        .unwrap();
