...
async fn task_detail_and_retry_endpoints_return_operational_state() {
    ...
    assert_eq!(
        retry_payload.get("status").and_then(Value::as_str),
        Some("queued")
    );
}
...
async fn project_operations_are_recorded_in_audit_log() {
    ...
    assert!(
        items
            .iter()
            .any(|item| item.get("action").and_then(Value::as_str) == Some("project.created"))
    );
    assert!(
        items.iter().any(
            |item| item.get("action").and_then(Value::as_str) == Some("source.import.enqueued")
        )
    );
}
  ...
  .unwrap();
    ...
    let neighbors_response = build_app(state.clone())
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/projects/{project_id}/graph/chain-of-thought/neighbors"
                ))
                .header(header::COOKIE, &cookie)
                .body(Body::empty())
                .unwrap(),
