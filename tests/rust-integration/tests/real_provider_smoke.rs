...
async fn real_provider_contract_exposes_models_and_chat_completion() -> Result<()> {
  if !smoke_enabled() {
    eprintln!("Skipping live provider smoke. Set KNOWLEDGE_RUN_REAL_PROVIDER_SMOKE=1 to run it.");
    return Ok(());
  }
  ...
  assert!(
    model_ids.iter().any(|value| *value == provider.model.as_str()),
  ...
  let completion = chat_completion(
    &client,
    &provider,
    "Reply with a single word: ready",
  )
  .await?;
...
async fn real_provider_query_and_save_round_trip() -> Result<()> {
  ...
  let task_id = create_query_task(&state, &cookie, &csrf, &project_id, "What is attention?").await?;
  let task = wait_for_task_terminal(&state, &task_id).await?;
  assert_eq!(task.status, "succeeded");

  let result = task.result.as_ref().context("missing query result")?;
  assert_eq!(
    result.get("citations").and_then(Value::as_array).map(Vec::len),
    Some(1)
  );
  assert_eq!(
    result
  ...
  let save_response = build_app(state.clone())
    .oneshot(
      Request::builder()
        .method("POST")
        .uri(format!("/api/projects/{project_id}/query-tasks/{task_id}/save"))
...
async fn real_provider_ingest_search_and_semantic_lint_smoke() -> Result<()> {
  ...
  assert_eq!(search_payload.get("mode").and_then(Value::as_str), Some("keyword"));
  assert_eq!(
    search_payload
      .get("vectorHits")
  ...
  let ingest_result = ingest_task.result.as_ref().context("missing ingest result")?;
  assert!(
    ingest_result
      .get("summaryPath")
      .and_then(Value::as_str)
  ...
  let lint_project_id =
    create_project(state.clone(), &cookie, &csrf, lint_root.clone()).await?;
  configure_provider(&state, &provider, None).await?;
...
fn provider_config() -> Result<ProviderConfig> {
  ...
  let model = std::env::var("KNOWLEDGE_PROVIDER_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
...
impl ProviderConfig {
  fn auth_headers(&self) -> Result<reqwest::header::HeaderMap> {
    ...
    let value = reqwest::header::HeaderValue::from_str(&format!("Bearer {}", self.api_key.trim()))
