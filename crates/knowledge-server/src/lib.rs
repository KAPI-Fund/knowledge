...
async fn seed_runtime_settings(pool: &PgPool, config: &AppConfig) -> anyhow::Result<()> {
  ...
  let Some((provider_mode, current_base_url, current_api_key, current_model, current_timeout_seconds)) =
    current
  else {
    return Ok(());
  };
  ...
  if provider_mode != "deterministic"
    && current_base_url.as_deref().is_some_and(|value| !value.trim().is_empty())
    && current_model.as_deref().is_some_and(|value| !value.trim().is_empty())
  {
    return Ok(());
  }
  ...
  .bind(config.provider_mode.clone().unwrap_or_else(|| "openai-compatible".to_string()))
  ...
  let admin_exists = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM users WHERE username = 'admin'")
    .fetch_one(pool)
    .await?;
