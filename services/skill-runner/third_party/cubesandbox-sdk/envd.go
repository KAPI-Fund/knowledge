...
func (s *Sandbox) startProcess(ctx context.Context, payload processStartRequest, opts CommandOptions) (*processStartResult, error) {
	...
	// Process.Start is a Connect server-streaming RPC: the request body must be a
	// single 5-byte-prefixed envelope (flag + big-endian length), not bare JSON.
	// envd rejects unframed bodies with "protocol error: promised N bytes".
	...
	req, err := s.newEnvdRequest(ctx, http.MethodPost, "/process.Process/Start", nil, &envelope)
	if err != nil {
		return nil, err
	}
...
func (e *processEndEvent) exitCode() (int, bool) {
	...
	// This envd omits the numeric exit code and only reports a Go-style status
	// string, e.g. "exit status 0" / "exit status 1".
	if code, ok := parseExitStatus(e.Status); ok {
		return code, true
	}
	// Exited cleanly with no decodable status: treat as success.
	if e.Exited && e.Status == "" {
		return 0, true
	}
	return 0, false
}

func parseExitStatus(status string) (int, bool) {
	const prefix = "exit status "
	if !strings.HasPrefix(status, prefix) {
		return 0, false
	}
	n, err := strconv.Atoi(strings.TrimSpace(status[len(prefix):]))
	if err != nil {
		return 0, false
	}
	return n, true
